...
public class GUI extends JFrame {
	...
	protected void tiedotTaulukkoon(Kirja[] kirjahylly) {
		...
		for (int row = 0; row < kirjahylly.length; row++) {
			...
			if (kirjahylly[row] != null) {
				...
				model.addRow(new Object[] { nimi, tekija, "" + vuosi });
	...
	protected void lisaaKirja() {
		...
		// Avataan uusi optionPane jossa k‰ytet‰‰n edell‰ luodun paneelin
		// kentti‰

		int result = JOptionPane.showConfirmDialog(null, myPanel, "Lis‰‰ kirja: ", JOptionPane.OK_CANCEL_OPTION);

		// VAIHTOEHTOISESTI VOIDAAN LUODA ILMENTYMƒ KOKONAAN UUDESTA FRAMESTA JA
		// LUKEA TIEDOT SEN AVULLA
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			JDBCExample.tallennaKirja(uusi);
